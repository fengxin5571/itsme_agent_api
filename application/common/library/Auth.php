...
use app\common\model\UserInviteLog;
...
class Auth
{
    ...
    protected $config = [];
    protected $options = [];
    protected $allowFields = ['id', 'username', 'nickname', 'mobile', 'avatar', 'score', 'city', 'worth'];
    ...
    public function init($token)
    {
        ...
        if ($user_id > 0) {
            ...
            $user = $redis::$handler->get('userinfo:'.$user_id);
            if(!$user){
                $user = Agent::get($user_id);
                Agent::setRedisAgentInfo($user);
            }else{
            ...
            // 非正常 非禁言
            if ($user->status != 'normal' && $user->status != '2') {
    ...
    public function register($username, $password, $email = '', $mobile = '', $extend = [])
    {
        ...
        Db::startTrans();
        try {
            $user = Agent::create($params, true);
            $user = Agent::get($user->id);
            User::setUserInfo($user);
            $redis = new RedisPackage();
            $redis_user = $redis::$handler->get('userinfo:'.$user->id);
            if($redis_user){
                $this->_user = json_decode($redis_user);
            }else{
            ...
            // 保存邀请会员记录
            if(isset($extend['p_user_id'])){
                UserInviteLog::create([
                    'user_id' => $extend['p_user_id'],
    ...
    public function changepwd($newpassword, $oldpassword = '', $ignoreoldpassword = false)
    {
        ...
        if ($this->_user->password == $this->getEncryptPassword($oldpassword, $this->_user->salt) || $ignoreoldpassword) {
            ...
            try {
                ...
                $this->_user->loginfailure = 0;
                $this->_user->password = $newpassword;
                $this->_user->salt = $salt;
                User::setUserInfo($this->_user);
                $this->_user->save();
    ...
    public function getUserinfo()
    {
        ...
        //$data = json_decode(json_encode($this->_user),true);
        //$allowFields = $this->getAllowFields();
        //$userinfo = array_intersect_key($data, array_flip($allowFields));
        //$userinfo = $data;
        Token::set($this->_token, $this->_user->id, $this->keeptime);
        $userinfo = Agent::getUserInfo($this->_user->id);
