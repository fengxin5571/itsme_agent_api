...
class Redis extends Driver
{
    ...
    {
        $options = [
            'host'       => Env::get('token_redis.host', '127.0.0.1'),
            'port'       => Env::get('token_redis.port', '6379'),
            'password'   => Env::get('token_redis.password', ''),
            'select'     => Env::get('token_redis.select', 2),
            'timeout'    => Env::get('token_redis.timeout', 0),
            'expire'     => Env::get('token_redis.expire', 0),
            'persistent' => Env::get('token_redis.persistent', false),
            'userprefix'     => Env::get('token_redis.userprefix', 'up:'),
            'tokenprefix'     => Env::get('token_redis.tokenprefix', 'tp:'),
        ];
    ...
    public function set($token, $user_id, $expire = 0)
    {
        ...
        $up_arr = $this->handler->SMEMBERS($this->getUserKey($user_id));
